...
BunGo completely removes the burden. It **auto-injects** the compiled JSX bundle (inline by default, or via a content-hashed `src="/_bungo/<view>.<hash>.js"` when `srv.SetAssetOptimization(true)` is enabled), serialized handler JSON (`window.__BUNGO_DATA__`), and (in dev) the live-reload client by inserting a snippet **before the first `</head>`** in the rendered HTML; if there is no `</head>`, it falls back to **before `</body>`**, or appends to the document if neither tag exists.
